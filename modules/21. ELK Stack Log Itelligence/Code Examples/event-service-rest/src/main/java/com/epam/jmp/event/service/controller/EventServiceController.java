...
@Log4j2
@RestController
@RequestMapping("/event-service/v3.0/events")
@Api(
        value = "event-service",
        description = "Set of endpoints for Creating, Retrieving, Updating and Deleting of Events.")
public class EventServiceController {

//    private static final Logger log = LogManager.getLogger(EventServiceController.class);
    ...
    public @ResponseBody
    ResponseEntity<?> getEvent(@ApiParam("Id of the Event to be obtained. Cannot be empty.")
                               @PathVariable("id") BigInteger id) {
        log.info("GET /{id} request start");
        log.warn("GET /{id} request warn message");
        log.debug("GET /{id} request debug message");
        log.error("GET /{id} request error message");
    ...
    public ResponseEntity<?> createEvent(@RequestBody Event event) {
        ...
        if (createdEvent != null) {
            URI location = ServletUriComponentsBuilder
                    .fromCurrentRequest().path("/{id}").buildAndExpand(createdEvent.getId()).toUri();
    ...
    public ResponseEntity<?> updateEvent(@PathVariable("id") BigInteger id, @RequestBody Event event) {
